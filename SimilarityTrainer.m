classdef SimilarityTrainer
    ...
    methods
        ...
        function [training, classification] = get_training_data_for_stock(first_stock_index, num_of_stocks_for_training, jump_interval)
            ...
            for second_stock = 1:num_of_stocks_for_training
                ...
                for j = TRAINING_DATA_START:-jump_interval:TRAINING_DATA_END
                    [training(index,:), classification{index, 1}] = get_training_and_classification_of_stocks(obj.ItsCsvMatrix(first_stock_index,:), obj.ItsCsvMatrix(second_stock,:), j, num_of_days_in_sample);
                    index = index + 1;
                end
            end
        end

        function [current_training, classification] = get_training_and_classification_of_stocks(first_stock, second_stock, index_of_first_value_in_training, num_of_days_in_training)
